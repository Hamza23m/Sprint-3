...
MODIFY cvv VARCHAR(20) NOT NULL; 
-- ---------------------------------
...
-- ---- Media de compra realizado por cada compañía. Ordenando de mayor a menor promedio de compra.
-- ------------------------------------------------------------------------------------------------------------------------------
...
GROUP BY c.company_name,
		 ...
		 c.country
; 
-- ------ Mostramos la vista ordenando de mayor a menor promedio de compras
SELECT *
...
-- ------ Insertamos los datos y comprobamos. 
SELECT *
FROM user;
-- ------ Cambiamos el nombre de la tabla.
RENAME TABLE user to data_user; 
-- ------ Cambiamos el nombre de la columna email.
...
SELECT * 
FROM company; 
-- ----- Eliminamos la FK credit_card_id.
ALTER TABLE transaction
DROP FOREIGN KEY FK_Credit_card;
-- ----- Cambiamos el type de la columna credit_card_id en la tabla transaction.
ALTER TABLE transaction
MODIFY credit_card_id varchar(20) NOT NULL;
-- ----- Cambiamos todos los types de la columnas de credit_card
ALTER TABLE credit_card
MODIFY id varchar(20) NOT NULL,
MODIFY iban varchar(50) NOT NULL,
MODIFY pin varchar(4) NOT NULL,
MODIFY cvv INT NOT NULL;
-- ----- Creamos una FK para relacionar "credit_card" con "transaction".
ALTER TABLE transaction
ADD CONSTRAINT FK_Credit_card
FOREIGN KEY (credit_card_id) REFERENCES credit_card(id);
-- ----- Añadimos en la tabla credit_card una nueva columna llamada fehca_actual.
-- ----- Le ponemos la fecha actual en la que la insertamos.
ALTER TABLE credit_card
ADD COLUMN fecha_actual DATE DEFAULT (CURRENT_DATE); 
-- ------- Mostramos la tabla.
SELECT * 
FROM credit_card;
-- ----- Eliminamos la vista vista_marketing.
DROP VIEW vista_marketing;
-- ------ Insertamos tambien el los datos del user la nueva empresa que hemos creado antriormente. 
INSERT INTO data_user
		(id, name, surname, phone, personal_email, birth_date, country, city, postal_code, address) 
VALUES  ('9999', 'Maria', 'Gutierrez', '034 4759 2148' ,'m.gutierrez@ttspepe.com', 'Jul 04, 1970',' Spain', 'Murcia', '030710', 'calle ciencia 24');
-- ------ Añadimos la FK para relacionar "data_user" con "transaction".
ALTER TABLE transaction
ADD CONSTRAINT FK_user_id 
FOREIGN KEY (user_id) REFERENCES data_user(id);
-- ------ Con esto ya tenemos el diagrama tal y como nos pide el ejercicio.

-- ------------------------------------------------------------------------------------------------------------------------------
-- N3.2 Crear una vista llamada "InformeTecnico" que contenga: ID de la transacción, Nombre del usuario, Apellido del usuario, 
-- ---- IBAN de la tarjeta de crédito usada, Nombre de la compañía. Asegúrese de incluir información relevante de las tablas que conocerá y 
-- ---- utilice alias para cambiar de nombre columnas según sea necesario. Ordena los resultados de forma descendente por ID de transacción.
-- ------------------------------------------------------------------------------------------------------------------------------
...
SELECT
    ...
    cc.iban  AS num_cuenta_bancaria,
...
-- ---- Mostramos la vista en orden descendente por id_transacción. 
SELECT *
